...
def parse_seq_rawdata(args, root_dir, save_dir, seq_name='scene-400', ID=None):
    ...
    vis_save_dir = os.path.join(seq_save_dir, 'vis')
    ...
    for camera_id, camera in enumerate(cameras_name):
        for frame_id_cur_cam, sample in enumerate(samples):
            ...
            for anno_token in sample['anns']:
                ...
                if x_min < -100 or y_min< -100 or x_max< -100 or y_max< -100:
                    continue
                if x_max>image_width*1.2 or y_max >image_height*1.2:
                    continue
                img_draw = cv2.rectangle(copy.deepcopy(img), (x_min, y_min), (x_max, y_max), (0, 0, 255),thickness=2)
                w = x_max - x_min
                h = y_max - y_min


                if w < 0.2 * image_width or h < 0.1 * image_height:  # limit box size
                    continue
                ...
                cv2.imwrite(os.path.join(vis_save_dir, '%06d.png'%num), cv2.cvtColor(img_draw, cv2.COLOR_RGB2BGR))
                num+=1
        ...
        for frame_id_cur_cam, sample in enumerate(samples):
            for anno_token in sample['anns']:
                ...
                if obj_id not in existing_id:
                    vis_save_dir =parse_seq_rawdata(args, root_dir, save_dir, scene_name, ID=obj_id)
                    pose_dir = os.path.join(os.path.dirname(vis_save_dir), "sparse/0")
                    if len(os.listdir(vis_save_dir)) == 0 or len(os.listdir(pose_dir)) == 0:
                        shutil.rmtree(os.path.dirname(vis_save_dir))

                    existing_id.append(obj_id)

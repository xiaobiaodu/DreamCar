...
class Zero123Guidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        sam_dir = os.path.join(self.cfg.cond_image_path, "sam")
        if os.path.exists(sam_dir):
            max_radius = 0
            max_box_idx = None
            max_box_w = 0

            img_list = glob.glob(os.path.join(sam_dir, "*rgba.png"))
            ...
            for i, img_name in enumerate(img_list):
                ...
                img_path = os.path.join(sam_dir, img_name)
            ...
            pose = np.linalg.inv(obj_poses[max_box_idx]) @ c2ws[max_box_idx]
            degrees = cartesian_to_spherical(pose[:3, -1])
            theta_deg = np.rad2deg(degrees[0])
            azimuth_deg = np.rad2deg(degrees[1])

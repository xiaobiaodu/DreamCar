...
class Mesh:
    ...
    def filter_bbox(self, min_corner, max_corner):
        inside = torch.all((self.v_pos >= min_corner) & (self.v_pos <= max_corner), axis=1)

        # Prepare a tensor for the new indices that matches the dtype and device of the original indices
        new_indices = torch.empty(inside.size(0), dtype=torch.long, device=inside.device)
        new_indices.fill_(-1)  # Fill with -1 or another placeholder to identify removed vertices

        # Assign new indices only to vertices that are inside the bbox
        new_indices[inside] = torch.arange(torch.sum(inside), dtype=torch.long, device=inside.device)

        # Filter vertices
        self.v_pos = self.v_pos[inside]

        # Update face indices to reflect the removed vertices
        # First, ensure all vertices in a face are inside the bbox
        all_vertices_inside = torch.all(inside[self.t_pos_idx], dim=1)
        filtered_faces = self.t_pos_idx[all_vertices_inside]

        # Map old vertex indices to new ones for the remaining faces
        # Only keep faces that do not contain -1 (i.e., all vertices were inside the bbox)
        self.t_pos_idx = new_indices[filtered_faces]
        valid_faces_mask = torch.all(self.t_pos_idx >= 0, dim=1)
        self.t_pos_idx = self.t_pos_idx[valid_faces_mask]

...
class TetrahedraSDFGrid(BaseExplicitGeometry):
    ...
    def isosurface(self) -> Mesh:
        ...
        # if self.cfg.crop_box:
        #     track_info = os.path.join(self.cfg.image_path, "track_info.txt")
        #     with open(track_info, "r") as f:
        #         track = f.read().splitlines()
        #
        #     data = track[1].split(" ")
        #     box_length = torch.from_numpy(np.array(data[4:7]).astype(float)).cuda()
        #     min_bbox_side = box_length.min()
        #     max_bbox_side = box_length.max()
        #
        #     # mesh_length = torch.max(self.mesh.v_pos, dim=0)[0] - torch.min(self.mesh.v_pos, dim=0)[0]
        #     mesh_length = torch.max(self.mesh.v_pos, dim=0)[0] * 2
        #     min_mesh_side = mesh_length.min()
        #
        #
        #     scale = min_bbox_side / min_mesh_side
        #
        #     # min_corner = torch.tensor([0.45,0,0.18]).cuda()  * -1
        #     # max_corner = torch.tensor([0.45,0.21,0.18]).cuda()
        #     y=0
        #     for i in range(3):
        #         if i == 0:
        #             continue
        #         if box_length[i] == max_bbox_side:
        #             continue
        #         y = box_length[i]
        #
        #     box = torch.tensor([max_bbox_side, y,box_length[0]]).cuda()
        #     # max_corner = box / scale /2
        #     # min_corner = box / scale /2 * -1
        #
        #     max_corner = box /2
        #     min_corner = box /2 * -1
        #     self.mesh.filter_bbox(min_corner, max_corner)
        return mesh

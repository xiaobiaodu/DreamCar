...
def get_rays(
    ...
    c2w = torch.unsqueeze(torch.unsqueeze(c2w,1).clone(), 1).clone()
    directions = torch.unsqueeze(directions, 3).clone().to(c2w)
    R = c2w[:, :, :, :3, :3].clone()
    T = c2w[:, :, :, :3, 3].clone()
    # rays_d = torch.sum(directions  * R, -1)    # (B, H, W, 3)
    rays_d_full = directions  * R

    rays_d = torch.sum(rays_d_full, dim=-1)
...
def get_mvp_matrix(
    ...
    w2c[:, :3, 3:] = -torch.matmul(c2w[:, :3, :3].transpose(1, 2), c2w[:, :3, 3:])
    w2c[:, 3, 3] = 1.0
    # calculate mvp matrix by proj_mtx @ w2c (mv_mtx)
    mvp_mtx = torch.matmul(proj_mtx.to(w2c), w2c)
    return mvp_mtx
